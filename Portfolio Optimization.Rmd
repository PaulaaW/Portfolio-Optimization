...
Here we take the real data from Yahoo.Finance, and calculate the real return of the portfolio and the market. 
...
In this section we will discuss the results or the outputs that are generated from the portfolio optimization process. Firstly, the average daily return of the stocks are all range from 0.1% to 0.45%, which are all positive and shows that these stocks are performed well during the last year. The covariance matrix indicates that the highest correlation is 0.1889 between Paypal and Shopify, and the lowest correlation is 0.0587 between Alibaba and Netflix. The correlation range of 0.0587 - 0.1889 is not high and we can say that the portfolio's risk is diversified pretty well. From the first random weight we generated, the resulted annualized return is 1.1, risk is 0.35, and the sharpe ratio is 3.15. This means that the portfolio has been increase its value more than one time within this year, the price volatility is 0.35, and there is 3.15 times of return for bearing 1 time of the risk. This investment result is pretty effective as the sharpe ratio is considered good when it is larger than 1. 

Next, we'll look at our 5000 simulations. The portfolio with the minimum variance is having a annualized return of 0.73, a risk of 0.3, and a sharpe ratio of 2.4. The portfolio with the maximum sharpe ratio is having a return of 1.6, a risk of 0.4, and a sharpe ratio of 3.8. To better understand the outputs of the simulation, we will bring the efficient frontier graph into our discussion. 
...
For the Monte Carlo Simulation, we will assume the investor is risk aversion and use the minimum variance portfolio generated above. The confidence interval of the forecasted portfolio return in the next 30 days is between -13.2% - 30.4%, with an average of 7.02%. And the confidence interval of the forecasted market performance is between -8.9% - 25.4%, with an average of 6.9%. From these indexes, the market is having a narrower range and an similar average than the optimized portfolio. Even though the portfolio has a higher upper bound than the market, it is more volatile and therefore we will go for the less risky one and say that the market will be more efficient and perform better in the next 30 days.
